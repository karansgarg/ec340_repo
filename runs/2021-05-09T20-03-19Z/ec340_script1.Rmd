...
```{r}
svm <- svm(X1 ~ X2 + X3 + X4 + X5 + X6 + X7 + X8 + X9 + X10 + X11 + X12 + X13 + X14 + X15 + X16 + X17 + X18 + X19 + X20 + X21, data=hsbc_train)
summary(svm)
```

```{r}
svm_gs <- GridSearchCV$new(svm,
                           parameters = list(cost = seq(from=0.1, to=1, by=0.1),
                                             gamma = seq(from=0.0001, to=1, by=0.005),
                                             epsilon = seq(from=0.01, to=0.1, by=0.01)),
                           n_folds = 5,
                           scoring = "rmse")
```

```{r}
svm_gs_fit <- svm_gs$fit(hsbc_train, "X1")
```
...
```
```{r}
accuracies <- rbind(linear_accuracy, rf_accuracy, xgb_accuracy, lstm_accuracy)
```
...
Limitations: SVM and others not considered. Walk-forward validation not considered (more realistic). Only most extreme form tested (we used only past prices and no other input features). 